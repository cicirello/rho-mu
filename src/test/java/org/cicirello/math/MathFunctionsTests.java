...
public class MathFunctionsTests {
  ...
  public void testLogGammaExtremes() {
    assertEquals(
        Double.POSITIVE_INFINITY, MathFunctions.logGamma(Double.POSITIVE_INFINITY), EPSILON);
    assertEquals(
        Double.NEGATIVE_INFINITY, MathFunctions.logGamma(Double.NEGATIVE_INFINITY), EPSILON);
    final double HUGE = 2.556348e305;
    final double LARGEST_NEGATIVE_NONINTEGER_D = -4.5035996273704955e15;
    final double TOO_NEGATIVE =
        LARGEST_NEGATIVE_NONINTEGER_D - Math.ulp(LARGEST_NEGATIVE_NONINTEGER_D);
    ...
    assertEquals(
        1.0e8 * (Math.log(1.0e8 + 0.5) - 1) + 0.4189385332,
        MathFunctions.logGamma(1.0e8 + 0.5),
        EPSILON);
