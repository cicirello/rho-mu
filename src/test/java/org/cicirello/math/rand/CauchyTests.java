...
public class CauchyTests extends SharedTestRandom {
  ...
  public void testNextCauchyThreadLocalRandom() {
    ...
    for (double s : scale) {
      double[] boundaries = {
        -s * (sqrt2 + 1), -s, -s * (sqrt2 - 1), 0, s * (sqrt2 - 1), s, s * (sqrt2 + 1)
      };
      ...
      for (int i = 0; i < buckets.length; i++) {
        assertTrue(
            buckets[i] > 0,
            "verifying at least 1 sample in each bucket, scale=" + s + " bucket=" + i);
      }
    }
    double median = 5.0;
    double s = 1.0;
    ...
    for (int i = 0; i < buckets.length; i++) {
      assertTrue(
          buckets[i] > 0, "verifying at least 1 sample in each bucket, median=5 bucket=" + i);
    }
  }
  ...
  public void testNextCauchySplittableRandom() {
    ...
    for (double s : scale) {
      ...
      assertTrue(
          chi <= 14.07,
          "Verifying chi square statistic below .95 critical value: chi=" + chi + " crit=14.07");
    }
    double median = 5.0;
    double s = 1.0;
    double[] boundaries = {
