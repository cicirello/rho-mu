 ...
 * The tests in this class specifically verify that methods that should 
 * delegate behavior to the wrapped object delegate to the correct method,
...
public class DelegationTests {
	...
	private static class FakeRNG implements RandomGenerator {
		...
		@Override public IntStream ints(int origin, int bound) {
			fail("Should not delegate int(origin, bound)");
			return IntStream.generate(() -> 0).sequential();
		}
		
		@Override public IntStream ints(long streamSize, int origin, int bound) {
			fail("Should not delegate int(origin, bound)");
		...
		@Override public DoubleStream doubles(double origin, double bound) {
			return DoubleStream.generate(() -> 4216.0).sequential();
		}
		
		@Override public DoubleStream doubles(long streamSize, double origin, double bound) {
